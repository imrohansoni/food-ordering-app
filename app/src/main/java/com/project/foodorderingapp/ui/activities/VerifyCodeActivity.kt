...
class VerifyCodeActivity : AppCompatActivity() {
    ...
    private fun verifyCodeButtonHandler(view: View) {
        if (code.length == 4) {
            ...
            val body = hashMapOf<String, Any>(
                "mobile_number" to sendCodeResponse!!.mobileNumber,
                "hash" to sendCodeResponse!!.hash,
                "expires_at" to sendCodeResponse!!.expiresAt,
    ...
    private suspend fun makeVerifyCodeRequest(body: HashMap<String, Any>) {
        try {
            val res = apiServices(context = applicationContext, tokenRequired = false).login(body)
            if (res.isSuccessful && res.body() != null) {
                AuthTokenManager.saveAuthToken(
                    applicationContext,
                    res.body()!!.data.authToken
                )
